...
const scroll_show = () => {
    ...
    scroll_top.value.classList.add('morad')
    scroll_top.value.classList.remove('hideScroll')
    } else {
        scroll_top.value.classList.add('hideScroll')
  }
}
...
onMounted(() => {
    window.addEventListener('scroll', scroll_show)
      ...
      const arandom= Math.random().toString(36).substr(2, 9); // generates a random 9-character string
...
<template>
    <div class="font-titilium scroll-sm relative">
        ...
        <Home_main/>
        <Footer/>
        <!-- <div class="fixed bottom-16 right-8 bg-green-800 p-3 rounded-full text-2xl border-4 border-white text-white z-[100]">
        ...
        <div class="scroll_top flex items-center justify-center bg-navbar_bg border-2 border-primary_color2 " ref="scroll_top" @click="goToTop()">
            ...
            <br>
            <RouterLink :to="{ name: 'Jobs' }">Go to jobs</RouterLink>
            <br>
      <Router-link :to="{ name: 'JobsDetail', params: { slugs: arandom } }">Job Detail</Router-link>
